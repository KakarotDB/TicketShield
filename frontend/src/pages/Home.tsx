  ...
  return (
    <div style={{ background: "var(--bg-primary)" }}>
      ...
      <div style={{ position: "relative", minHeight: 520, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center", padding: "70px 40px 90px", overflow: "hidden" }}>
        <div style={{ position: "absolute", bottom: -60, left: "50%", transform: "translateX(-50%)", width: 600, height: 300, background: "radial-gradient(ellipse at center bottom, #7c3aed 0%, #4c1d95 30%, transparent 70%)", opacity: .35, borderRadius: "50%" }} />
        <div style={{ position: "absolute", bottom: -40, left: "50%", transform: "translateX(-50%)", width: 360, height: 180, background: "radial-gradient(ellipse at center bottom, #a78bfa 0%, #7c3aed 40%, transparent 70%)", opacity: .5, borderRadius: "50%" }} />
        <div style={{ position: "absolute", bottom: -80, left: -60, width: 420, height: 260, background: "radial-gradient(ellipse, #6d28d9 0%, #4c1d95 35%, transparent 70%)", opacity: .25, borderRadius: "50%" }} />
        <div style={{ position: "absolute", bottom: -80, right: -60, width: 420, height: 260, background: "radial-gradient(ellipse, #6d28d9 0%, #4c1d95 35%, transparent 70%)", opacity: .25, borderRadius: "50%" }} />
      ...
      <div className="concert-bg-section">
        ...
        <div className="concert-bg-content">
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 18, marginBottom: 52 }}>
            {[
              { icon: "🎪", num: "01", numColor: "var(--purple-mid)", cls: "glass-card", iconCls: "card-icon card-icon-purple", title: "Organiser creates event", desc: "Sets face price and resale cap permanently on-chain. Nobody — not even the organiser — can change it after deployment." },
              { icon: "🎫", num: "02", numColor: "#60a5fa", cls: "glass-card", iconCls: "card-icon card-icon-blue", title: "Fan buys ticket", desc: "Receives a unique token in their Phantom wallet. Payment goes directly to the organiser — no middleman." },
              { icon: "⛔", num: "03", numColor: "var(--purple-bright)", cls: "glass-card-purple", iconCls: "card-icon card-icon-purple-bright", title: "Scalper gets rejected", desc: "The contract rejects any resale above the cap. No appeal. No loophole. The code has no complaints department." },
            ].map((c, i) => (
